...
export async function fetchGLDashboard() {
  const response = await apiFetch("/gl/dashboard", { credentials: "include" });
  return parseJson<GLDashboard>(response);
}

export async function fetchGLAccounts() {
  const response = await apiFetch("/gl/accounts", { credentials: "include" });
  return parseJson<GLAccount[]>(response);
}

export async function createGLAccount(payload: Partial<GLAccount>) {
  const response = await apiFetch("/gl/accounts", jsonInit("POST", payload));
  return parseJson<GLAccount>(response);
}

export async function updateGLAccount(id: number, payload: Partial<GLAccount>) {
  const response = await apiFetch(`/gl/accounts/${id}`, jsonInit("PUT", payload));
  return parseJson<GLAccount>(response);
}

export async function deleteGLAccount(id: number) {
  const response = await apiFetch(`/gl/accounts/${id}`, { method: "DELETE", credentials: "include" });
...
export async function fetchGLJournalEntries(params: { status?: string; source?: string; start?: string; end?: string } = {}) {
  ...
  const response = await apiFetch(`/gl/journal-entries${suffix}`, { credentials: "include" });
  return parseJson<GLJournalEntry[]>(response);
}

export async function createGLJournalEntry(payload: GLJournalEntryInput) {
  const response = await apiFetch("/gl/journal-entries", jsonInit("POST", payload));
  return parseJson<GLJournalEntry>(response);
}

export async function postGLJournalEntry(id: number) {
  const response = await apiFetch(`/gl/journal-entries/${id}/post`, { method: "POST", credentials: "include" });
  return parseJson<GLJournalEntry>(response);
}

export async function voidGLJournalEntry(id: number) {
  const response = await apiFetch(`/gl/journal-entries/${id}/void`, { method: "POST", credentials: "include" });
  return parseJson<GLJournalEntry>(response);
}

export async function deleteGLJournalEntry(id: number) {
  const response = await apiFetch(`/gl/journal-entries/${id}`, { method: "DELETE", credentials: "include" });
  return parseJson<{ id: number; deleted: boolean }>(response);
}
...
export async function fetchGLEntryBySource(refType: string, refId: number) {
  ...
  const response = await apiFetch(`/gl/journal-entries/by-source?${query.toString()}`, { credentials: "include" });
...
export async function fetchGLSourceMap(refType: string, refIds?: Array<number | string>) {
  ...
  const response = await apiFetch(`/gl/journal-entries/source-map?${query.toString()}`, { credentials: "include" });
...
export async function createGLEntryFromSource(payload: GLEntryFromSourceInput) {
  const response = await apiFetch("/gl/journal-entries/from-source", jsonInit("POST", payload));
  return parseJson<GLJournalEntry>(response);
}

export async function fetchGLCashSummary() {
  const response = await apiFetch("/gl/cash-summary", { credentials: "include" });
...
export async function fetchGLTrialBalance(params: { start?: string; end?: string } = {}) {
  ...
  if (params.end) query.set("end", params.end);
  const suffix = query.toString() ? `?${query.toString()}` : "";
  const response = await apiFetch(`/gl/reports/trial-balance${suffix}`, { credentials: "include" });
...
export async function fetchGLAccountLedger(accountId: number, params: { start?: string; end?: string } = {}) {
  ...
  if (params.end) query.set("end", params.end);
  const suffix = query.toString() ? `?${query.toString()}` : "";
  const response = await apiFetch(`/gl/reports/account-ledger/${accountId}${suffix}`, { credentials: "include" });
