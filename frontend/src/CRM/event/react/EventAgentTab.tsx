...
type Plan = {
  ...
  tasks?: { add?: unknown[]; update?: unknown[]; delete?: unknown[] };
  budget?: { add?: unknown[]; update?: unknown[]; delete?: unknown[] };
  flow?: { add?: unknown[]; update?: unknown[]; delete?: unknown[]; reorder?: unknown[] };
};
...
function planHasContent(p?: Plan | null): boolean {
  if (!p) return false;
  const g = (o?: { add?: unknown[]; update?: unknown[]; delete?: unknown[]; reorder?: unknown[] }) =>
    (o?.add?.length || 0) + (o?.update?.length || 0) + (o?.delete?.length || 0) + (o?.reorder?.length || 0);
  return Boolean((p.settings && Object.keys(p.settings).length) || g(p.tasks) || g(p.budget) || g(p.flow));
}
...
function summarizePlan(p: Plan): string[] {
  ...
  const seg = (label: string, o?: { add?: unknown[]; update?: unknown[]; delete?: unknown[]; reorder?: unknown[] }) => {
    if (!o) return;
    const parts: string[] = [];
    if (o.add?.length) parts.push(`加 ${o.add.length}`);
    if (o.update?.length) parts.push(`改 ${o.update.length}`);
    if (o.delete?.length) parts.push(`删 ${o.delete.length}`);
    if (o.reorder?.length) parts.push("重排序");
...
export function EventAgentTab({ eventId, canEdit, isMobile, onApplied }: { eventId: number; canEdit: boolean; isMobile: boolean; onApplied?: () => void }) {
  ...
  useEffect(() => {
    ...
    const room = `event_agent_${eventId}`;
    ...
    socket.on("event_agent_reply", (payload: { job_id?: string; status?: string; reply?: string; plan?: Plan | null; message?: string }) => {
  ...
  if (!canEdit) {
    return <div style={emptyStyle}>活动 Agent 需要 event_edit 权限。</div>;
  }

  const examples = ["预算合计多少？还有哪些待办没完成？", "加一个待办：检查帐篷是否完好，负责人 Tony", "流程里午餐后加一个 30 分钟的分享环节"];
    ...
    <div style={wrapStyle}>
      <div style={headRowStyle}>
        <div>
          <div style={{ fontWeight: 800, fontSize: 15 }}>
            <i className="fa-solid fa-robot" style={{ marginRight: 6, color: "var(--x-color-accent)" }} />活动 Agent
          </div>
          <div style={mutedStyle}>能读/改本活动的 基本设置 · 待办 · 预算 · 流程（不新建活动）。改动会先给你预览，点「应用」才执行。</div>
        </div>
      ...
      <div style={inputBarStyle}>
        <textarea
          ...
          placeholder="对活动下指令或提问…（Enter 发送，Shift+Enter 换行）"
          style={textareaStyle}
          rows={isMobile ? 2 : 3}
        />
