...
- [x] 手机端活动签到升级为 QR 流程：`我要签到` 默认打开相机扫描别人临时码，`和我签到` 生成 5 分钟临时码，网页端签到名单显示帮签人。
- [x] 手机端导出 / 下载入口统一改为下载 Blob 后触发系统分享，覆盖相册、简章/附件、表单 Excel/QR、法会文件、文件系统、报销、资产 CSV 和 APK 下载。
- [x] 增加 `/event/{event_id}` 动态分享 HTML，meta-data 和 icon/og:image 自动使用活动相册 cache 图片，并跳转到 `/#/event/{event_id}`。
