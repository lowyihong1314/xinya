"""活动数据的查询与写入（原 backend/app/event/services.py）。

只做框架适配：``jsonify(x), code`` → ``json_response(x, code)``、
``current_app.config`` / ``env_value`` → ``settings``、``send_file`` → starlette 响应、
``flask_login.current_user`` → ``core.auth.current_user``。
校验顺序、状态码、中文文案、响应体的键名与嵌套形状**逐字照搬** ——
活动页 / 报名页 / 财政页三处前端都在按这些键和 message 分支。
...
── 搬迁时被迫改掉的 import（不是风格，是会把 Flask 拉回进程）────────────
  ...
  · ``from werkzeug.utils import secure_filename``（在 budget_pdf.py 里）
    → ``backend.core.files``。
  · QR 签名密钥从 ``current_app.config["SECRET_KEY"]`` 换成 ``settings.secret_key``
    —— 后者就是 core/auth.py 给会话 Cookie 签名用的那一个，同一进程内自洽即可
    （这个令牌只活 5 分钟，不跨进程也不落库）。

── 两处**仍然会把 Flask 拉回进程**的惰性 import（已知欠账，不是疏漏）────
...
── 「看着像 bug、故意保留」清单 ──────────────────────────────────────
  ...
  · ``month_events_response``：``year=0`` 或 ``month=0`` 走的是 ``not year`` 那一支，
    回 400 "Invalid year or month" —— 这串**是英文**，与本文件其它文案不统一。
    前端可能在按它匹配，不要翻译。
  · ``all_event_response``：``per_page = min(per_page, 100)`` 只封顶不封底，
    ``?per_page=-1`` 会原样传给 paginate。原样保留。
  · ``save_event``：新建时 ``datetime`` 必填、编辑时不传就保持原值，这条不对称是
    故意的（编辑弹窗可以只改文字）。
  · ``save_event`` 的整个函数体裹在一个 ``except Exception → 500 str(exc)`` 里，
    所以里面的 ``int(item)`` 之类抛出来会变成一句裸异常文案回给前端。照搬。
  · ``set_event_album``：``album`` 传任意真值都算 True（``bool(album)``），
    传字符串 "false" 也是 True。照搬 —— 前端发的是真 JSON 布尔。
  · ``delete_event_unit`` / ``save_event_unit`` 用的是 ``.query.get`` 而不是
    ``get_or_404``，所以 404 的响应体是 ``{"status","message"}`` 而不是通用 404。
  · ``_can_edit_event_flow`` 里那个 ``try/except Exception: pass`` 会把权限查询
    失败静默当成「没权限」，然后落到「是不是组织者」那一支。照搬。
"""
...
from backend.models.event_data import (
    AlbumFiles,
    EventBudgetData,
    EventCheckIn,
    EventData,
...
from backend.models.finance import ManualIncome, ReimbursementRequest
from backend.models.user_data import User

from .storage import DATA_PATH, to_short_data_path
...
# 原来这里有一个 ``get_json_payload()``（``request.json or request.form or {}``）。
# 请求体的读取归 FastAPI 管了，所以它搬到了 router.py 的 ``_JSON_BODY`` 参数上；
# 那个表达式在 Flask 3.1 下的真实行为（request.form 分支其实是死代码）写在
# router.py 的模块头里。
...
def _check_in_qr_serializer():
    # 原来是 current_app.config["SECRET_KEY"]；见模块头。
    return URLSafeTimedSerializer(settings.secret_key, salt=CHECK_IN_QR_SALT)
...
def viewer_sees_hidden_events() -> bool:
    ...
    否则筹备中的活动在后台也会凭空消失。"""
    # ``current_user`` 换成了 core.auth 的 ContextVar 代理。它的 __bool__ 是
    # 「有没有登录用户」（flask_login 的代理即便匿名也是 truthy），
    # 所以 ``current_user and current_user.is_authenticated`` 的结果两边一致。
...
def month_events_response(year, month):
    if not year or not month or month < 1 or month > 12:
        # 文案是英文，与本文件其它地方不一致 —— 照搬，见模块头。
        return json_response({"status": "error", "message": "Invalid year or month"}, 400)
...
def _save_event_attachment_file(event, uploaded_file):
    # uploaded_file 是 uploads.UploadedFile（werkzeug FileStorage 的替身）：
    # ``.filename`` / ``.mimetype`` / ``.save(path)`` 三样都按原语义提供。
...
def save_event_check_in(data):
    ...
    if not event_id:
        return json_response({"status": "error", "message": "event_id 必填"}, 400)
    if not user_id:
        return json_response({"status": "error", "message": "user_id 必填"}, 400)
    if current_user.id == user_id:
        return json_response({"status": "error", "message": "不可以帮自己 check-in"}, 400)
    if valid_user_id is not None and valid_user_id == user_id:
        return json_response(
            {"status": "error", "message": "valid_user_id 不可以和 user_id 相同"}, 400
        )

    event = EventData.query.get_or_404(event_id)
    user = User.query.get(user_id)
    if not user:
        return json_response({"status": "error", "message": "user 不存在"}, 404)

    validator = None
    if valid_user_id is not None:
        validator = User.query.get(valid_user_id)
        if not validator:
            return json_response({"status": "error", "message": "valid_user 不存在"}, 404)
    ...
    return json_response({"status": "success", "message": message, "data": record.to_dict()})
...
def scan_event_check_in_qr(data):
    ...
    if not raw_token:
        return json_response({"status": "error", "message": "缺少签到 QR code"}, 400)

    try:
        payload = _check_in_qr_serializer().loads(raw_token, max_age=CHECK_IN_QR_SECONDS)
    except SignatureExpired:
        return json_response(
            {"status": "error", "message": "签到 QR code 已过期，请对方重新生成"}, 400
        )
    except BadSignature:
        return json_response({"status": "error", "message": "无效的签到 QR code"}, 400)

    token_event_id = as_int(payload.get("event_id"))
    owner_id = as_int(payload.get("user_id"))
    if not token_event_id or not owner_id:
        return json_response({"status": "error", "message": "签到 QR code 内容不完整"}, 400)
    if event_id and event_id != token_event_id:
        return json_response({"status": "error", "message": "这个 QR code 不属于当前活动"}, 400)
    if owner_id == current_user.id:
        return json_response({"status": "error", "message": "不可以扫描自己的签到码"}, 400)

    event = EventData.query.get_or_404(token_event_id)
    owner = User.query.get(owner_id)
    if not owner:
        return json_response({"status": "error", "message": "签到码拥有者不存在"}, 404)
...
def delete_event_check_in(check_in_id):
    ...
    db.session.commit()
    return json_response({"status": "success", "message": "Check-in 已删除", "id": check_in_id})
...
def create_event_flow(data):
    ...
    if not _can_edit_event_flow(event):
        return json_response({"status": "error", "message": "没有权限编辑此活动流程"}, 403)
    max_no = (
    ...
    db.session.add(flow)
    db.session.commit()
    return json_response({"status": "success", "message": "Flow 已创建", "data": flow.to_dict()})
...
def update_event_flow(flow_id, data):
    ...
    db.session.commit()
    # ⚠️ 注意这里返回的是**整个 event** 的 to_dict_full()，不是 flow.to_dict() ——
    #    和上面的创建接口不对称。前端更新后直接拿它重刷整页，照搬。
    return json_response(
...
def delete_event_flow(flow_id):
    ...
    try:
        ...
        db.session.commit()
        return json_response({"status": "success", "message": "Flow 已删除并重新排序"})
...
def reorder_event_flow(data):
    ...
    if not isinstance(flow_ids, list) or not flow_ids:
        return json_response({"status": "error", "message": "flow_ids 必须是非空数组"}, 400)
    ...
    for flow_id in flow_ids:
        if flow_id not in flow_map:
            return json_response(
                {"status": "error", "message": f"flow_id 不属于该 event：{flow_id}"},
                400,
            )
...
def delete_event_by_id(event_id):
    ...
    if event.album_files and len(event.album_files) > 0:
        return json_response(
            {"status": "error", "message": "该活动仍有相册文件，不能删除"},
            400,
        )
    ...
    try:
        ...
        db.session.commit()
        return json_response({"status": "success", "message": f"Event {event_id} 已删除"})
    except Exception as exc:
        db.session.rollback()
        return json_response({"status": "error", "message": str(exc)}, 500)
...
def save_event_unit(form, files):
    """form / files 是 uploads.FormValues / 普通 dict（werkzeug MultiDict 的替身）。

    签名不动是为了让函数体逐字照搬 —— 特别是 ``form.get("event_id", type=int)``
    那条「转不动就当没传」的语义，见 uploads.py 模块头 ③。
    """
    ...
    if role not in EVENT_UNIT_ROLES:
        return json_response(
            {"status": "error", "message": f"role 必须是 {' / '.join(EVENT_UNIT_ROLES)}"}, 400
        )
    if not unit_name:
        return json_response({"status": "error", "message": "单位名称必填"}, 400)

    if unit_id:
        unit = EventOrganizingUnit.query.get(unit_id)
        if not unit:
            return json_response({"status": "error", "message": "单位不存在"}, 404)
    else:
        event = EventData.query.get(event_id) if event_id else None
        if not event:
            return json_response({"status": "error", "message": "Event 不存在"}, 404)
        max_sort = (
    ...
    db.session.commit()
    return json_response({"status": "success", "message": "进行单位已保存", "data": unit.to_dict()})
...
def reorder_event_units(data):
    ...
    if not isinstance(unit_ids, (list, tuple)) or not unit_ids:
        return json_response({"status": "error", "message": "unit_ids 必填"}, 400)

    try:
        ordered_ids = [int(uid) for uid in unit_ids]
    except (TypeError, ValueError):
        return json_response({"status": "error", "message": "unit_ids 必须是整数列表"}, 400)

    units = EventOrganizingUnit.query.filter_by(event_id=int(event_id)).all()
    units_by_id = {unit.id: unit for unit in units}
    if set(ordered_ids) != set(units_by_id.keys()):
        return json_response(
            {"status": "error", "message": "unit_ids 与该活动的进行单位不匹配"}, 400
        )
...
def delete_event_unit(unit_id):
    ...
    db.session.commit()
    return json_response({"status": "success", "message": "已删除"})
    ...
    try:
        ...
        if is_edit:
            ...
            if not data.get("event_name"):
                return json_response({"status": "error", "message": "event_name 必填"}, 400)
        ...
        if not is_edit:
            ...
            if not start_dt:
                return json_response({"status": "error", "message": "datetime 必填"}, 400)
            event.datetime = start_dt
        elif "datetime" in data and str(data.get("datetime")).strip():
            start_dt = parse_datetime(data.get("datetime"))
            if not start_dt:
                return json_response({"status": "error", "message": "datetime 格式错误"}, 400)
            event.datetime = start_dt
        ...
        if "end_datetime" in data:
            ...
            if raw is None or str(raw).strip() == "":
                ...
                if not end_dt:
                    return json_response(
                        {"status": "error", "message": "end_datetime 格式错误"}, 400
                    )
        ...
        db.session.commit()
        return json_response({"status": "success", "message": "Event 已保存", "data": event.to_dict()})
    except Exception as exc:
        db.session.rollback()
        return json_response({"status": "error", "message": str(exc)}, 500)
...
def set_event_album(event_id, album):
    event = EventData.query.get_or_404(event_id)
    if album is None:
        return json_response({"status": "error", "message": "missing album"}, 400)
...
def upload_event_brochure(event_id, uploaded_file):
    ...
    try:
        ...
        if extension not in BROCHURE_EXTENSIONS:
            return json_response(
                {"status": "error", "message": "简章仅支持 PDF、PPT、PPTX、XLS、XLSX、DOC、DOCX、DOX"},
                400,
            )
        ...
        db.session.commit()
        return json_response({"status": "success", "message": "简章已上传", "data": event.to_dict()})
    except ValueError as exc:
        db.session.rollback()
        return json_response({"status": "error", "message": str(exc)}, 400)
    except Exception as exc:
...
def set_event_brochure(event_id, file_id):
    ...
    try:
        ...
        if file_id in (None, "", 0, "0"):
            ...
            db.session.commit()
            return json_response({"status": "success", "message": "已取消简章", "data": event.to_dict()})

        record = EventFile.query.get(as_int(file_id))
        if not record or record.event_id != event.id:
            return json_response({"status": "error", "message": "附件不存在或不属于该活动"}, 404)
        extension = os.path.splitext(record.file_name or record.file_path or "")[1].lower()
        if extension not in BROCHURE_EXTENSIONS:
            return json_response(
                {"status": "error", "message": "只有文档类附件（PDF/PPT/Word/Excel）可设为简章"},
                400,
            )
        event.brochure_file_id = record.id
        db.session.commit()
        return json_response({"status": "success", "message": "已设为简章", "data": event.to_dict()})
    except Exception as exc:
        db.session.rollback()
        return json_response({"status": "error", "message": str(exc)}, 500)
...
def upload_event_file(event_id, uploaded_file):
    try:
        ...
        db.session.add(record)
        db.session.commit()
        return json_response({"status": "success", "message": "活动附件已上传", "data": record.to_dict()})
    except ValueError as exc:
        db.session.rollback()
        return json_response({"status": "error", "message": str(exc)}, 400)
    except Exception as exc:
...
def delete_event_file(file_id):
    ...
    try:
        ...
        return json_response({"status": "success", "message": "活动附件已删除", "id": file_id})
    except Exception as exc:
        db.session.rollback()
        return json_response({"status": "error", "message": str(exc)}, 500)
...
def event_task_list_response(event_id):
    ...
    return json_response({"status": "success", "data": [t.to_dict() for t in tasks]})
...
def create_event_task(data):
    ...
    if not title:
        return json_response({"status": "error", "message": "事项不能为空"}, 400)
    ...
    db.session.add(task)
    db.session.commit()
    return json_response({"status": "success", "data": task.to_dict()})
...
def _fahui_income_rows(event, include_pending=False):
    ...
    """
    from backend.models.fahui import FahuiOrder, FahuiOrderItem, FahuiPayment, fahui_payment_order
...
def _quote_header_value(value):
    """werkzeug ``http.quote_header_value`` 的复刻（**不 import werkzeug**）。

    只在下面那个 Content-Disposition 里用。规则：空串加引号、全是 token 字符时
    原样输出、否则转义反斜杠与双引号再加引号。
    """
    value = str(value)
    if not value:
        return '""'
    if _HEADER_TOKEN_CHARS.issuperset(value):
        return value
    value = value.replace("\\", "\\\\").replace('"', '\\"')
    return f'"{value}"'


# werkzeug http.py 的 _token_chars，逐字符照抄。
_HEADER_TOKEN_CHARS = frozenset(
    "!#$%&'*+-.0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ^_`abcdefghijklmnopqrstuvwxyz|~"
)


def _attachment_disposition(download_name):
    """复刻 Flask ``send_file(as_attachment=True, download_name=...)`` 的那个头。

    照抄 werkzeug/utils.py:send_file：文件名是纯 ASCII 时只写 ``filename=``；
    含中文时同时写一个 NFKD 降级的 ASCII ``filename=`` 和 RFC 5987 的
    ``filename*=UTF-8''…``（``quote`` 的 safe 集合是 RFC 5987 的 attr-char）。

    ★ 为什么不直接用 starlette 的 ``FileResponse(filename=...)``：
      ① 这里的内容是内存里的 BytesIO，不是磁盘文件；
      ② starlette 只写 ``filename*=``、不写 ASCII 降级的 ``filename=``。
      这个头决定用户存盘看到的文件名（"xxx_财政报告.pdf"），换一套写法就是换文件名。
    """
    try:
        ...
        return (
            f"attachment; filename={_quote_header_value(simple)}; "
            f"filename*=UTF-8''{quoted}"
        )
    return f"attachment; filename={_quote_header_value(download_name)}"
...
def event_budget_report_pdf_response(event_id):
    ...
    # 原来是 send_file(buffer, mimetype="application/pdf", as_attachment=True,
    # download_name=filename)。starlette 没有等价物，所以自己拼头（见 _attachment_disposition）。
    # Content-Length 由 Response 按 body 长度自动补，与 Flask 一致。
    return Response(
        buffer.getvalue(),
...
def create_event_budget(data):
    ...
    if not category:
        return json_response({"status": "error", "message": "类别不能为空"}, 400)
    ...
    db.session.add(item)
    db.session.commit()
    return json_response({"status": "success", "data": item.to_dict()})
...
def _google_geocoding_key():
    # 原来是 env_value("GOOGLE_GEOCODING_API_KEY")：取到空值时返回 None。
    # 这里保持「空 → None」，因为下面两处都是 ``if not key`` 分支，语义一致。
    return (settings.google_geocoding_api_key or "").strip() or None


def place_autocomplete_response(query):
    q = (query or "").strip()
    if not q:
        return json_response({"status": "success", "predictions": []})
    key = _google_geocoding_key()
    if not key:
        return json_response({"status": "error", "message": "未配置 Google API key"}, 500)
    try:
        resp = requests.get(
        ...
        return json_response({"status": "error", "message": f"Google 请求失败：{exc}"}, 502)
    status = data.get("status")
    if status not in ("OK", "ZERO_RESULTS"):
        return json_response(
            {"status": "error", "message": data.get("error_message") or status or "查询失败"}, 502
        )
    preds = [
    ...
    return json_response({"status": "success", "predictions": preds})


def place_detail_response(place_id):
    pid = (place_id or "").strip()
    if not pid:
        return json_response({"status": "error", "message": "缺少 place_id"}, 400)
    key = _google_geocoding_key()
    if not key:
        return json_response({"status": "error", "message": "未配置 Google API key"}, 500)
    try:
    ...
    if data.get("status") != "OK":
        return json_response(
            {"status": "error", "message": data.get("error_message") or data.get("status") or "查询失败"},
            502,
        )
...
def maps_config_response():
    # 原来是 env_value("VITE_GOOGLE_MAPS_EMBED_API_KEY")；env_value 会剥掉 VITE_ 前缀
    # 再查 settings，所以对应的字段就是 google_maps_embed_api_key
    # （它的 AliasChoices 里带着那个 VITE_ 环境变量名，取值来源没变）。
    return json_response(
        {"status": "success", "embed_key": (settings.google_maps_embed_api_key or "").strip()}
    )
