...
原 ``backend/app/event/budget_pdf.py``，搬迁时**只改了一行 import**：
``from werkzeug.utils import secure_filename`` → ``backend.core.files``
（后者是 werkzeug 那个函数的逐字节复刻，见 core/files.py 的模块头）。
绘图逻辑、字体回退、金额格式一个字符没动 —— 这份 PDF 会被打印存档，
版面变了对不上以前的报告。
"""
...
# 为一个纯字符串函数把整个 werkzeug 拖进进程不划算，何况它属于 Flask 栈。
