"""问答游戏（Kahoot 式）的对局引擎。原 backend/app/quiz_game/services.py，**逐字平移**。

题库在 SQL（models.quiz_game）里，而**一场正在进行的对局整个活在 Redis 里** ——
这样 4 个 uvicorn worker 里谁接到请求都读得到同一份状态（和 api/quiz 的抢答墙同一套路数）：

    quizg:token:{token}        一个 JSON，就是整场游戏（题目快照 / 状态 / 当前题号 / 截止时刻）
    quizg:{token}:players      hash，guest_id → 一名玩家（名字 / 分数 / 连对 / 在线）
    quizg:{token}:answers      hash，guest_id → 本题的作答（hsetnx 保证一人一次）
    quizg:{token}:scored:{i}   第 i 题的计分闸（SET NX），见下面 ③
    quizg:sid_map              hash，连接 id → "{token}|{guest_id}"，断线时按它把人标成离线

每题的倒计时由**主持人的客户端**驱动，服务端没有任何定时器；服务端只在「在场的人
都答完了」时顺手自动揭晓一次。

── 搬迁改了什么 ────────────────────────────────────────────────────────
只动 import，函数体一个字节没改：
  · ``from backend.models import db`` 与 ``QuizGameQuestion`` 两个 import 删掉 ——
    本文件从头到尾没有一处 ``db.``，也没用过 ``QuizGameQuestion``，
    留着只会白白多牵一条依赖。真正在用的只有 ``QuizGameSet``。

── ★ 几处「看着像 bug、但故意保留」 ────────────────────────────────────

  ① ``record_answer`` 的「全员答完」判定是 ``answered >= online_count(token)``，
     而 ``online_count`` 只数 ``online=True`` 的人。掉线的人不进分母，所以有人锁屏时
     剩下的人一答完就立刻揭晓 —— 这是有意的（否则一个掉线的人能把全场卡住）。
     反过来，SSE 的掉线判定本来就不及时，可能迟迟凑不满而不自动揭晓，
     主持人手上的「揭晓」按钮是兜底，两条路都要留着。

  ② ``start_question(token, index)`` 在 ``index >= len(questions)`` 时直接转 ``show_podium``。
     空题库在 ``create_session`` 就被 ``empty_set`` 拦掉了，所以这条分支实际只有
     ``next_question`` 翻过最后一题时才走到 —— 看着像「开始游戏会跳颁奖台」，实际到不了。

  ③ ``reveal`` 的防重复计分用的是 ``scored:{index}`` 这个 **SET NX** 闸，而不是把
     「已计分」写回 session。因为 session 是「读出来改完再整个写回去」，不是原子的，
     两个 worker 同时 reveal 会互相覆盖。这个闸是整套计分逻辑里**唯一**的并发保护，别拿掉。

  ④ ``_my_result`` 判「我答对了没」用 ``a.get("choice") == q.get("answer")``，
     和 ``reveal`` 里真正加分的判断是同一个表达式。两处必须同时改，
     否则会出现「界面说你答对了，但分没加」。

  ⑤ ``socket_room()`` 现在只剩 backend/app/ 那边的 Socket.IO 调用方在用。
     SSE 的频道由 ``core.realtime.channel_of("quiz_game", token)`` 自己拼，
     **千万别**把这个函数的返回值当 room_id 传给 publish_sync ——
     那会拼成 ``rt:quiz_game:quizg:{token}``，订阅端永远收不到。
     等 Socket.IO 整体下线，它和 ``sid_map_key`` 一起删。
"""
...
from backend.models.quiz_game import QuizGameSet
