"""资产 / 库存的业务逻辑（原 backend/app/asset/services.py，逐行照搬）。

模型分三层：``AssetItem``（物品）→ ``AssetSubItem``（尺码/颜色等具体条目，库存的最小单位）
→ ``AssetInventory``（某仓库里某个子 item 的数量）。所有数量变动**只能**经由
``AssetStockDocument``（单据）确认时产生 ``AssetStockMovement``（流水）来发生 ——
没有任何一条路径能直接改 ``AssetInventory.quantity``，``update_inventory_threshold``
改的也只是 min_quantity（预警线），不是库存本身。

搬迁只动了三行 import：``backend.models.db`` → ``backend.core.db.db``、
包内 ``app.asset.*`` → ``api.asset.*``。**其余一个字节没改** —— 校验顺序、中文错误文案、
返回字典的键名和嵌套形状都是前端在按着分支的契约。

★ 一串「看着像 bug、故意保留」的地方，改之前先想清楚：

  ① ``_user_label`` 那档死分支见 serializers.py 的说明。

  ② ``_generate_next_warehouse_code`` / ``_generate_next_item_code`` 是「查最大号 + 1，
     撞了就往上加」的循环，**没有加锁**。两个请求同时建仓库时理论上会抢同一个号，
     靠 code 的唯一约束兜底（后提交的那个 commit 抛 IntegrityError → 路由的
     ``except Exception`` → rollback + 500）。并发量在这个场景下极低，保持原样。
     TODO(资产): 真要修就上一张序号表或 SELECT ... FOR UPDATE，不要在这里打补丁。

  ③ ``_optional_int`` 把字符串 ``"null"`` 也当成「没传」。这是前端某处把 JS 的 null
     拼进字符串造成的，服务端在兜。别删 —— 删了那个页面会开始报「格式错误」。

  ④ ``_delete_relative_data_file`` 用 ``DATA_ROOT / Path(relative_path)`` 拼路径。
     pathlib 的规则是「右边是绝对路径就丢掉左边」，所以库里万一存了绝对路径，
     这里会删到 DATA_ROOT 之外，而且整个函数用 ``except Exception: pass`` 吞掉。
     现状下 invoice_file_path 只由 ``upload_document_invoice`` 写入（永远是相对路径），
     构不成实际问题。TODO(安全): 收口时加一层「必须在 DATA_ROOT 下」的判定。

  ⑤ ``_normalize_upload_name`` **故意不用** secure_filename：发票文件名常含中文，
     werkzeug 那套会把中文整个清空（见 core/files.py 的说明）。这里只做
     basename + 去控制字符，然后落盘名是
     ``asset_document_{单据id}_{8位随机}_{stem}{ext}`` —— 目录分隔符已经在
     basename 一步被去掉，拼不出跨目录的路径，安全性由那一步保证。

  ⑥ ``_validate_document_payload`` 里 ``"invoice_file_path" in payload`` 是
     **「键在不在」而不是「值是不是空」**：请求体不带这个键时沿用单据上的旧值，
     带了（哪怕是 null）才覆盖。改成 ``payload.get(...)`` 会让每次编辑单据都清掉发票文件。

  ⑦ 同一个函数里，``document_type`` 不属于 {purchase_in, sale_out, sale_return} 时
     **无条件清空** invoice_no / reference_type / reference_id —— 前端传了也丢掉。
     这是业务规则（只有采购/销售单才谈得上发票和报销引用），不是漏写。

  ⑧ ``update_inventory_threshold`` 在缺 ``min_quantity`` 字段时是 ``int(None)``
     → TypeError → ValidationError「最低库存 必须是整数」（中间那个空格来自 f-string，
     照抄，前端可能在按整句匹配）。

  ⑨ ``create_warehouse(payload, user)`` 收了 user 但没用、也没 ``del user``；
     其余同类函数都写了 ``del user``。纯风格不一致，保持原样。

  ⑩ ``load_asset_documents_data`` / ``load_asset_movements_data`` 的 limit 默认 30 且
     **路由层不透传**，即前端固定只拿最近 30 条、没有翻页。这是现状，不在本次迁移范围内。

  ⑪ ``cancel_stock_document`` 对 draft 单据直接改状态、不回滚库存（draft 本来就没产生流水）；
     对 confirmed 单据是「按 id 倒序逐条生成一条 movement_type='cancel' 的反向流水」，
     **不是删掉原流水**。所以流水表是只增不减的账，别「优化」成删除。
"""
...
from backend.api.asset.exceptions import NotFound, ValidationError
...
# 原来是 from backend.models import db —— 那是 core.db 的同一个对象的再导出。
# 直接从 core.db 取，少走一层 backend.models 的包初始化。
...
from backend.models.finance import ReimbursementRequest
