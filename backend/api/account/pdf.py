"""报销单 / 收款报表 / Payment Voucher 的 PDF 生成（原 backend/app/account/pdf.py）。

**这个文件从来就不依赖 Flask**（只有 reportlab + core.paths），所以搬迁时
一个字节没改，只补了这段 docstring。三个入口都返回一个已经 ``seek(0)`` 过的
``BytesIO``，由 router.py 包成 HTTP 响应：

    build_payment_report_pdf(payments)        收款审核列表 → 一页汇总表
    build_claim_report_pdf(claims)            报销单批量导出（每单一页 + 末页汇总）
    build_payment_voucher_pdf(data, approver) 单张 Payment Voucher（含双方签名）

★ 两处「看着奇怪但别动」：

  ① ``PDF_FONT`` 和 ``PDF_FONT_BOLD`` 是**同一个** STSong-Light。reportlab 自带的
     CID 字体里没有对应的粗体，硬换成别的字体会让中文直接变成黑方块。
     代码里所有 ``setFont(PDF_FONT_BOLD, ...)`` 的地方靠字号和位置做视觉区分。

  ② ``_safe_attachment_path`` 的 ``except AttributeError`` 分支是给
     Python < 3.9 的 ``is_relative_to`` 兜底的。我们跑在 3.12，这支走不到，
     但它是唯一一道「附件路径不能跑出 DATA_ROOT」的护栏，删了没好处。
"""
import os
