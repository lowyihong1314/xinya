"""报销申请 + 财政收款的业务逻辑（原 backend/app/account/services.py，逐行照搬）。

两块内容住在同一个文件里（原样保留）：

  · **报销（claim）**：提交 / 列表 / 编辑 / 审批 / 撤回 / 附件 / 删除，
    以及 Payment Voucher 的分享链接、公开签名、PDF 上下文。
  · **财政收款（finance payments）**：把报名(form/membership/youth_class)、
    法会(ylp/lamp)、销售(sales)、手动收款(manual) 四路来源**聚合成一张列表**，
    靠 id 偏移量区分命名空间：手动 3e9 > 销售 2e9 > 法会 1e9 > 报名（原样 id）。

搬迁改了什么（其余一个字节没动 —— 校验顺序、中文错误文案、返回字典的键名和
嵌套形状都是前端在按着分支的契约）：

  ① 包内 import ``app.account.*`` → ``api.account.*``；
     ``backend.models.db`` → ``backend.core.db.db``（同一个对象的再导出）。
  ② ``flask_login.current_user`` → ``core.auth.current_user``（ContextVar 代理，同接口）。
  ③ ``flask.jsonify(...)`` → ``core.responses.json_response(...)``，见下面 ★③。
  ④ ``os.environ.get("READ_BILL_DEFAULT_MODEL", "auto")`` → ``settings.read_bill_default_model``
     （core/config.py 那个 validator 已经做了 strip().lower()，和原式等价）。
  ⑤ ``request.host_url`` → ``core.urls.absolute_url``，见下面 ★②。

★ 五处「看着像 bug / 想顺手改，但必须原样」的地方 ──────────────────

  ① **``update_finance_payment_status`` 会把 form / membership / youth_class 三个
     scope 转发到还没搬的 Flask 模块**（app/form/services.py、
     app/user_control/membership.py）。那两个模块顶上 ``from flask import jsonify``，
     函数体里又真的在调 ``jsonify()`` —— 在 FastAPI 进程里没有 Flask 应用上下文，
     那一调用会抛 RuntimeError，这条路径**现在是坏的**。
     ⚠️ 这不是本次搬迁改坏的，而是跨模块依赖：账务列表里报名/会员/青少年那三类
        收款的「改状态」按钮，要等 form 和 user_control 搬完才能恢复。
     TODO(接线): form / user_control 搬完后，把这两处 import 换成 api.* 的对应函数，
        并确认它们返回的是 starlette Response（不是 Flask 的 (Response, code) 元组）。
     同一段里法会那一支（_update_fahui_finance_payment_status）是**能跑的**：
     它调的 payment_review.set_payment_review_status 自己不碰 flask，
     只是所在模块顶上 import 了 flask —— 代价是运行到这条路径时 flask 会被拉进进程。
     ``_serialize_fahui_finance_payment`` 里的 ``normalize_fahui_payment_status``
     同理（它是 ``/account/payments`` 列表的必经之路，所以这个代价是常态，
     直到 app/fahui 搬完）。这与 api/public_api/router.py 里那处法会 import 是同一笔账。

  ② **Payment Voucher 的分享链接**原来是 ``urlsplit(request.host_url)`` 拼出来的
     ``https://<Host 头>/#/payment-voucher-sign/<token>`` —— 它**不带 BASE_PATH**，
     在 /UTBA_DEMO 这种前缀下点开是 404；而且 Host 头是客户端可伪造的。
     这里换成 ``core.urls.absolute_url``（配置里的 APP_PUBLIC_ORIGIN 优先，
     前缀从 X-Forwarded-Prefix 补回来），与 api/email 的验证链接是同一个处置。
     前端 PaymentVoucherModal.tsx 是把 share_url **原样**复制到剪贴板 /
     ``window.open`` 的，所以它必须是完整可点地址。

  ③ **四个 ``_update_*_finance_payment_status`` 原本 ``return jsonify({...})``**
     —— 也就是说它们返回的是 HTTP 响应而不是数据，路由只负责透传。
     这个分层很难看，但改成「返回 dict、由路由包」会连带改掉上面①那三条转发路径
     （它们返回的是 Flask 响应），所以原样保留，只把 jsonify 换成 json_response。

  ④ ``delete_manual_finance_payment`` 的 ``real_id`` 算了偏移量的**两种情况**
     （>= 3e9 减偏移，否则原样）。而 ``_update_manual_finance_payment_status``
     只减不判。于是「传一个小 id 去删」会删掉 ManualIncome 表里的同号记录。
     TODO(潜在越权，本次不修): 前端只会传带偏移量的 id，但这条路由没有拦。

  ⑤ ``build_payment_report_context`` 里 ``int(pid)`` 没有 try —— 传个 "abc"
     直接 ValueError → 路由的 except Exception → 500（而不是 400）。
     旁边 ``build_claim_report_context`` 同样的地方是有 try 的。两者不一致是原状。
"""
...
from backend.api.account.exceptions import NotFound, PermissionDenied, ValidationError
from backend.api.account.permissions import resolve_user_permissions, user_can_manage_claims, user_can_read_all_claims
...
from backend.models.form import RegisForm, RegisPayment
from backend.models.event_data import EventData
...
from backend.models.user_data import Department
...
# 原式：os.environ.get("READ_BILL_DEFAULT_MODEL", "auto").strip().lower()。
# core/config.py 的 _normalize_read_bill_model 已经做了同样的 strip().lower()，
# 默认值也是 "auto"，所以这里等价 —— 差别只有「.flaskenv 不再是配置来源」。
READ_BILL_DEFAULT_MODEL = settings.read_bill_default_model
...
def _payment_voucher_public_url(token):
    """Payment Voucher 公开签名页的完整地址（给人复制 / 直接 window.open）。

    原式是 ``urlsplit(request.host_url)`` 取 scheme+netloc 再拼 —— 见模块 docstring ★②：
    那样拼出来的地址**不带 BASE_PATH**（/UTBA_DEMO 下点开是 404），
    而且 Host 头是客户端可伪造的（有人拿自己的 Host 打这个接口就能让我们生成
    指向他域名的"签名链接"）。absolute_url 优先用配置里的 APP_PUBLIC_ORIGIN，
    前缀从 X-Forwarded-Prefix 补回来。

    路径里的 ``#`` 是 SPA 的 hash 路由，public_url 只在前面拼前缀，不碰它。
    """
    return absolute_url(f"/#/payment-voucher-sign/{token}")
...
def submit_public_payment_voucher_signature(token, payload):
    ...
    return {
        "message": "签名已确认",
        "claim": data,
        # 旧值写死 "/api/account/..."；/api 这一段整体没了，照抄会给出一个 404 链接。
        # 过一遍 public_url：① 不再有 /api；② 自动补上项目前缀（BASE_PATH 为空时
        # 逐字节退化成 "/account/print_payment_voucher/public/<token>/download"）。
        # 前端 PaymentVoucherSignPage.tsx 其实是自己按 token 拼这个地址的，没在读这个键，
        # 但留着的值必须是对的 —— 别的调用方（APK / 分享出去的链接）只有它可用。
        "download_url": public_url(
            f"/account/print_payment_voucher/public/{request_obj.public_token}/download"
        ),
    }
...
def _list_sales_finance_payments(status=None):
    from backend.models.asset import AssetStockDocument
...
def _update_sales_finance_payment_status(payment_id, data):
    ...
    db.session.commit()
    # 返回的是**响应对象**而不是数据 —— 路由只负责透传，见模块 docstring ★③。
...
def _update_fahui_finance_payment_status(payment_id, data):
    # current_user 从 flask_login 换成 core.auth 的 ContextVar 代理（同接口）。
    # ⚠️ payment_review 所在的包顶上还 import 着 flask —— 走到这条路径时 flask 会被
