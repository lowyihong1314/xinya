...
凭据从 .flaskenv 换到了 core.config（旧键名在 core/config.py 里留了 AliasChoices）：
  - cloudflare_email_routing_token     ← Email_Routing_Address_API_KEY_Cloudflare
  - cloudflare_email_routing_zone_id   ← Zone_ID_Cloudflare   （utba.my 的 Zone ID，规则用）
  - cloudflare_account_id              ← Account_ID_Cloudflare（账号 ID，目的地址用）
...
def _cf(path):
    """拼 Cloudflare API URL。base_url 末尾有没有斜杠都行。"""
    return f"{settings.cloudflare_base_url.rstrip('/')}{path}"


def cf_config():
    """三者缺一就返回 None，configure_forwarding 会带「未配置」warning 直接返回。"""
    token = settings.cloudflare_email_routing_token
    zone_id = settings.cloudflare_email_routing_zone_id
...
def ensure_destination_address(config, email):
    ...
    listing = requests.get(
        _cf(f"/accounts/{account_id}/email/routing/addresses"),
        headers=headers,
        params={"per_page": 50},
        timeout=15,
    )
...
def set_forwarding_rule(config, username, email, old_rule_id=None):
    ...
    该权限：配了 cloudflare_email_routing_rules_token 就用它，否则退回地址 token。
    ⚠️ 现网没配这个值，所以实际一直走的是回落分支（地址 token 对 zone 级返 403）。
    """
    rules_token = settings.cloudflare_email_routing_rules_token or config["token"]
    ...
    if old_rule_id:
        try:
            requests.delete(
                _cf(f"/zones/{zone_id}/email/routing/rules/{old_rule_id}"),
                headers=headers,
                timeout=15,
            )
        except Exception:
    ...
    payload = {
        "matchers": [{"type": "literal", "field": "to", "value": f"{username}@{settings.email_domain}"}],
        "actions": [{"type": "forward", "value": [email]}],
        "enabled": True,
        "name": f"forward {username}@{settings.email_domain}",
        "priority": 0,
    }
    response = requests.post(
        _cf(f"/zones/{zone_id}/email/routing/rules"),
        headers=headers,
        json=payload,
        timeout=15,
    )
