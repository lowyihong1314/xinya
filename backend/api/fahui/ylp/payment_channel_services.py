"""法会收款方式（扫码 / 银行转账）的增删改查。

原 backend/app/fahui/YLP/payment_channel_services.py。业务逻辑一个字没动，
搬迁只换了一处：``werkzeug.utils.secure_filename`` → ``backend.core.files``
的逐字节复刻（本包不 import werkzeug）。

★ ``form`` / ``files`` 两个参数收的不再是 werkzeug 的 MultiDict / FileStorage，
  而是 api/fahui/uploads.py 里的 FormValues / FormFiles —— ``.get()`` 的语义
  （含「转不动就当没传」）和 ``file_storage.save(path)`` 都复刻过，函数体不用改。
"""
...
from backend.models.fahui import FahuiPaymentChannel
