...
原 backend/app/fahui/YLP/order_log.py。只换了两个名字的来源：
``flask_login.current_user`` → ``core.auth`` 的 ContextVar 代理，
``flask.has_request_context`` → common/session_state.py 的同名函数
（它判的是 core.auth 的 _current_request ContextVar，在后台线程 / CLI 里同样是 False）。
"""

from backend.core.auth import current_user
from backend.models import db
from backend.models.fahui import FahuiOrderLog

from ..common.session_state import (
    current_session_phone,
    current_verified_phones,
    has_request_context,
)
