...
原 backend/app/fahui/YLP/raw_docs.py。扫描、去重、匹配、OCR 那些逻辑一个字没动，
搬迁只动了取图那一条（``raw_doc_file_response``）：``flask.send_file`` 换成
api/fahui/downloads.py 里那份同名复刻（Content-Disposition / Cache-Control /
ETag 逐字节对拍过），调用行本身只多了一个 ``request=`` 参数。

★ 那个参数是**新增**的：``conditional=True`` 要靠请求头里的 ``If-None-Match`` /
  ``If-Modified-Since`` 判 304，而 FastAPI 没有 Flask 那种全局 request。
  路由把 ``Request`` 传进来即可；不传就永远回 200（不会出错，只是少了 304）。

★ ``save_uploaded_raw_docs(files)`` 收的不再是 werkzeug 的 FileStorage 列表，
  而是 api/fahui/uploads.py 的 UploadedFile（``.filename`` / ``.read()`` 都复刻过），
  函数体不用改。
"""
...
def list_raw_docs() -> dict:
    ...
    from backend.models.fahui import FahuiRawDoc as _Doc  # 局部导入，保持本模块可被单独引用
...
def raw_doc_file_response(filename: str, request=None):
    """按文件名取图；只认 images/ 底下的单层文件名，挡掉路径穿越。

    request 只用来判 304（见模块头），不传也能正常出图。
    """
    ...
    return send_file(target, conditional=True, max_age=3600, request=request)
