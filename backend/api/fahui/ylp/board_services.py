"""看板（大板贴牌位）与订单牌位条目的全部业务逻辑。

原 backend/app/fahui/YLP/board_services.py。1600 行业务逻辑一个字没动，
搬迁只换了两样：

  ① ``flask_login.current_user`` → ``core.auth`` 的 ContextVar 代理。
  ② 两处出向推送换成 core.realtime：
        socket_broker.emit("fahui:order_updated", d, room="fahui:order:<id>")
            → publish_sync(REALTIME_APP, "fahui:order:<id>", "fahui:order_updated", d)
        socket_broker.emit("fahui:board_changed", d)  （不带 room = 全局广播）
            → publish_sync(REALTIME_APP, BROADCAST_ROOM, "fahui:board_changed", d)
     房间名、事件名、payload 形状全部照搬，理由见 api/fahui/realtime.py。
     原来那两圈 ``try/except: pass`` 去掉了 —— publish_sync 自己吞异常、只返回 bool。

★ 本文件所有对外函数都返回 ``(dict, status_code)`` 元组，由路由那边转成
  ``json_response(payload, status_code)``。这个形状不要改 —— 板位相关的接口
  有十几个调用点在按 ``success`` / ``status`` 两套键分支（历史遗留的重复，照搬）。
"""
...
from backend.api.fahui.realtime import BROADCAST_ROOM, REALTIME_APP
...
from backend.models.fahui import (
    FahuiBoardData,
    FahuiBoardHeader,
    FahuiItemFormData,
    FahuiOrder,
...
def _notify_order_updated(order_id: int | None) -> None:
    ...
    if not order_id:
        return
    publish_sync(REALTIME_APP, f"fahui:order:{order_id}", "fahui:order_updated", {"order_id": order_id})
