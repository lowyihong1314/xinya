"""牌位 PDF 的生成器：把订单项目套进模板底图、排版、压缩、（可选）打条码。

原 backend/app/fahui/YLP/print_generator.py。排版逻辑一个字没动，搬迁只动了
``generate_paiwei_using_order_ids`` 那一个函数的三行出口 ——
它是全文件唯一直接返回 HTTP 响应的地方（其余函数返回 BytesIO）：

    jsonify({...}), 400  → json_response({...}, 400)
    flask.send_file(...)  → api/fahui/downloads.py 里那份**同名复刻**
                            （Content-Disposition / Cache-Control 都对拍过，
                             调用行一个字没改）
"""
...
def generate_paiwei_using_order_ids(order_ids, need_barcode=False):
    ...
    if not items:
        return json_response({"status": "error", "message": "没有找到对应的订单数据"}, 400)

    filtered_data = filter_fahui_data(items)
    if not filtered_data:
        return json_response({"status": "error", "message": "没有有效的法会数据"}, 400)
