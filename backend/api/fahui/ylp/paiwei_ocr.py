...
原 backend/app/fahui/YLP/paiwei_ocr.py。业务逻辑（提示词、JSON 提取、错误分支）
一个字没动，只把取配置的方式从 ``env_value("BYTEPLUS_*")`` 换成 ``settings.*``，
取值顺序逐条对齐（做法同 api/event/ark.py）：

  · ``_api_key``      = BYTEPLUS_API_KEY or ARK_API_KEY，再 strip
    —— ``settings.byteplus_key`` 这个 property 就是为了还原这条顺序才有的。
  · ``_vision_model`` = BYTEPLUS_VISION_MODEL or BYTEPLUS_MODEL or 默认值
    —— 对应 ``settings.byteplus_vision_model_name``（config.py 里 byteplus_vision_model
    的默认值**故意留空**，就是为了保住这条优先级）。
  · ``_base_url`` / 模型名的 pydantic 默认值与下面 DEFAULT_* 常量**逐字相同**，
    所以 ``or DEFAULT_*`` 那层兜底留着不会改变结果（留着是防有人把配置项设成空串）。
"""
