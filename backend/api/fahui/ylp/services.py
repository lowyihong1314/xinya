"""YLP 订单的序列化、搜索、建单，以及「谁能看这张单」那套规则。

原 backend/app/fahui/YLP/services.py。业务逻辑一个字没动，搬迁只换了三样东西：

  ① ``flask_login.current_user`` → ``core.auth`` 的 ContextVar 代理。
  ② 会话（已验证手机号）从 ``flask.session`` 换成 common/session_state.py。
  ③ 出向推送 ``socket_broker.emit("fahui:order_created", payload)``（**不带 room**
     的全局广播）→ ``publish_sync(REALTIME_APP, BROADCAST_ROOM, ...)``，见 api/fahui/realtime.py。
     原来那圈 ``try/except: pass`` 去掉了 —— publish_sync 自己吞异常、只返回 bool，
     语义一致（推送失败不能让建单失败）。

★ 一处**看着像 bug、故意保留**：``get_orders_by_phone`` 里
  ``logged_in = bool(current_user and current_user.is_authenticated)`` ——
  只要**登录了**就能按任意手机号查单，不校验法会读权限。这是公开页与后台共用
  同一条接口的后果，照搬。
"""
...
from backend.api.fahui.common.session_state import current_session_phone, current_verified_phones
...
def create_order_shell(data: dict) -> tuple[dict, int]:
    ...
    # 经 Redis 广播，任意 worker 发出去都能送达订阅端；
    # CRM 订单列表监听此事件即时插入新订单。
    # 事件名和 payload 的形状一个字都没动，只是通道从 Socket.IO 换成了 SSE 频道。
    publish_sync(
        REALTIME_APP,
        BROADCAST_ROOM,
        "fahui:order_created",
