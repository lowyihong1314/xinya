"""法会付款的统一序列化与审核流程（lamp + YLP 共用，靠 ``type`` 区分）。

原 backend/app/fahui/common/payment_review.py。校验顺序、状态码、中文文案、
响应体的键名与嵌套形状全部逐字照搬 —— 审核页在按 ``success`` / ``status`` /
``payment`` 这几个键分支，两套键名（``success`` 布尔 + ``status`` 字符串）
是历史遗留的重复，**故意保留**。

搬迁只动了三处：``jsonify(x), N`` → ``json_response(x, N)``；
``flask_login.current_user`` → ``core.auth`` 的 ContextVar 代理；
``from backend.app.fahui.YLP.order_log`` → ``backend.api.fahui.ylp.order_log``。

★ ``_refresh_registration_status`` 是一个**空函数**（只有 return），不是没写完：
  点灯登记的工作流状态已与付款解绑，注释就在函数体里。照搬，别删也别补逻辑。
"""
...
from backend.models.lamp_registration import Lamp, LampRegistration
...
def get_payment_detail(payment_id: int, *, payment_type: str | None = None):
    ...
    if not payment:
        return json_response({"success": False, "status": "error", "message": "支付记录不存在"}, 404)
    return json_response(
        {
            "success": True,
            "status": "success",
...
def set_payment_review_status(
    ...
    *,
    ...
    if payment.payment_type == PAYMENT_TYPE_YLP:
        from backend.api.fahui.ylp.order_log import log_order_change
...
def update_payment_review(
    ...
    *,
    ...
    try:
        ...
        db.session.rollback()
        return json_response({"success": False, "status": "error", "message": str(exc)}, 400)
    except Exception as exc:
        db.session.rollback()
        return json_response({"success": False, "status": "error", "message": str(exc)}, 500)
...
def delete_payment_record(payment_id: int, *, payment_type: str | None = None):
    ...
    return json_response({"success": True, "status": "success"})
