...
原 backend/app/form/form_agent.py。

── 搬迁改动的三处，与 ai_grouping.py 是同三条 ────────────────────────
  ① ``from flask import jsonify`` → ``core.responses.json_response``。
  ② 子进程的模块路径 ``backend.app.form.form_agent_worker``
     → ``backend.api.form.form_agent_worker``。**字符串**，改漏了静默失效
     （接口照回 200 + job_id，前端永远等不到 form_agent_reply）。
  ③ ``socket_broker.emit`` → ``publish_sync("form", room, …)``。房间名
     ``form_agent_{form_id}`` 一个字没改（它同时是 ``/agent/chat`` 响应体里的
     ``room`` 字段），见 realtime.py 模块头。

★ 「看着像 bug、故意保留」：
  · ``apply_form_agent_plan`` 里 ``ef.get("add")`` 循环每一轮都重算一次 ``maxno``，
    但新加的行要 flush 之后才进 ``form.extra_field_configs`` —— 一次加多个字段时
    order 会重复。排序只按 (order, id)，重复 order 的表现是「同 order 内按 id 排」，
    不影响正确性。照搬不修。
  · 同一个函数里 ``fees.add`` 建 RegistrationFee 时**没有**传 ``fee_scope``，
    落库拿的是列默认值。而手工加报名费的 ``service.add_fee`` 显式传了
    ``FORM_FEE_SCOPE``。两条路径不一致是既有事实，照搬。
    TODO(form): 确认列默认值确实是 "form" 之后再决定要不要对齐 —— 现在改等于
    改掉 AI 建出来的报名费在收款审核里的归属。
  · ``summary`` 是一串中文标签的列表，``count`` 是 ``len(summary)``；
    同一个动作重复出现就重复计数（比如分了 30 个人就有 30 个「分组」）。照搬。
"""
import json
import re
import subprocess
...
from .service import _extract_field_switches, _normalize_max_members
...
def _spawn_worker(job_payload):
    process = subprocess.Popen(
        # ★ 模块路径跟着包一起搬了，见模块头 ②。
        [sys.executable, "-m", "backend.api.form.form_agent_worker"],
...
def run_form_agent_job(job_payload):
    ...
    ``publish_sync`` 自己吞异常只返回 bool，所以这里不检查返回值 —— 与原来
    socket_broker.emit 失败时的处境一致：子进程没有别的出口（stderr 被 DEVNULL 吞）。
    """
