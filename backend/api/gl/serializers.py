"""总账模块的 JSON 序列化（原 backend/app/gl/serializers.py，一字未改）。

这里每一个键名都是前端在读的字段，**改名即线上故障**。几处容易被「顺手优化」掉的：

  · ``debit`` / ``credit`` 走 ``_num(...) or 0.0``：None 和 0 都出 0.0，
    前端因此可以直接做算术，不用先判空。别改成 ``_num(...)`` 让 null 漏出去。
  · ``is_cash`` 在分录里没有科目时出 ``False`` 而不是 ``None`` —— 同理，保持布尔。
  · ``created_by_name`` 按 display_name → name_NRIC → username 的顺序回退，
    三个都没有就是 None。顺序是有意的（显示名优先），别按字段表顺序重排。
