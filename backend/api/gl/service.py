"""总账 / 现金日记账的业务逻辑（原 backend/app/gl/services.py，逐行照搬）。

会计模型（原英文 docstring 照译）：全部复式记账 —— 一张凭证至少两条分录，
借方合计 == 贷方合计。现金 / 银行账户就是普通科目打上 ``is_cash`` 标记，
所谓「现金日记账」不过是这些科目的明细账。余额一律按**借方为正**存放
（借 +、贷 -），对外展示时的自然方向由科目类型推出来。

搬迁只动了三行 import：``flask_login.current_user`` → ``core.auth.current_user``、
``backend.models.db`` → ``backend.core.db.db``、包内 ``app.gl.*`` → ``api.gl.*``。
**其余一个字节没改** —— 校验顺序、中文错误文案、返回字典的键名和嵌套形状都是
前端在按着分支的契约。

★ 几处「看着像 bug、故意保留」的地方，改之前先想清楚：

  ① ``DEBIT_NORMAL_TYPES`` 导进来了但全文没用到。留着是因为它是 models.gl 的公开常量，
     删掉等于悄悄收窄本模块的导出面；真要清理请连同 models 那边一起看。

  ② ``_generate_entry_no`` 是「查最大号 + 1，撞了就往上加」的循环，**没有加锁**。
     两个请求同时开新凭证时理论上会抢同一个号，靠 entry_no 的唯一索引兜底（后提交的
     那个 commit 会抛 IntegrityError → 500）。并发量在这个场景下极低，保持原样。
     TODO(总账): 真要修就上一张序号表或 SELECT ... FOR UPDATE，不要在这里打补丁。

  ③ ``map_entries_by_source`` 的注释写「newest wins」，但 query 是 ``order_by(id.asc())``
     且同一个 ref_id 后写覆盖前写 —— 实际效果确实是「最大 id 最后写入」= 最新的赢，
     注释和代码是一致的，只是绕。别顺手把 asc 改成 desc，那会反过来变成「最旧的赢」。

  ④ ``_money`` 对 ``None`` / ``""`` 返回 0.00，而对 ``"abc"`` 抛 422。于是「分录里漏填
     借方」和「借方填了 0」走同一条分支，都靠后面的「借贷不能都为 0」拦。保持。

  ⑤ ``list_journal_entries`` 的 ``limit`` 既可能是 int 也可能是 str（来自 query string），
     里面统一 ``int(limit)``。传 ``""`` 时 ``if limit:`` 为假 → **不加 limit**（全量返回），
     这不是笔误，路由层依赖这个分支，见 router.py 的说明。
"""
...
# current_user 从 flask_login 换成 core.auth 的 ContextVar 代理：
# 同名同语义（.is_authenticated / .id），_current_user_id() 一个字不用改。
...
from backend.api.gl.exceptions import NotFound, ValidationError
from backend.api.gl.serializers import (
    serialize_account,
