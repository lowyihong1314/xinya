# api/gl — General Ledger + Cash Book

> v3：已从 Flask 蓝图搬到 FastAPI（原 `backend/app/gl/`）。
> `routes.py` → `router.py`，`services.py` → `service.py`，其余文件原样平移。
> URL 少了 `/api` 这一段：`/api/gl/...` → `/gl/...`。
...
`service.post_journal_from_source(source, ref_type, ref_id, lines, ...)` lets operational
