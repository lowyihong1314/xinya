...
from backend.app.account.exceptions import AccountError
from backend.app.account.pdf import build_claim_report_pdf, build_payment_report_pdf, build_payment_voucher_pdf
from backend.app.account.permissions import (
...
from backend.app.account.services import (
...
@account_bp.post("/payments/report")
def download_payment_report():
    try:
        user = require_claim_list_permission()
        payload = request.get_json(silent=True) or {}
        payments = build_payment_report_context(payload.get("payment_ids") or [], user)
        pdf_buffer = build_payment_report_pdf(payments)
        return send_file(
            pdf_buffer,
            mimetype="application/pdf",
            as_attachment=True,
            download_name=f"PaymentReport_{len(payments)}.pdf",
        )
    except AccountError as exc:
        return _error_response(exc)
    except Exception as exc:
