...
from backend.app.media.paths import (
    BROKEN_IMAGE_PATH,
    DATA_PATH,
    abs_data_path,
...
from backend.app.media.utils import ensure_jpeg_cache_file, get_duration, is_video_valid, remove_jpeg_cache_file
from backend.app.media.video_tasks import current_video_tasks
...
def _safe_emit(event, data):
    try:
        ...
        emitter.emit(event, data, namespace="/")
...
def _emit_event_room(event_code, action, payload=None):
    ...
    try:
        emitter = socketio if getattr(socketio, "server", None) is not None else socket_broker
        emitter.emit("media_room_update", message, namespace="/", to=room)
    except Exception as exc:
