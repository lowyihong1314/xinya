"""移动端会话令牌：签发 / 刷新 / 吊销 / 反解用户。

原 app/mobile/session_service.py。放在 core/ 而不是 api/mobile/ 下，是因为
core/auth.py 的 Bearer 解析要调 ``load_user_from_access_token`` —— 放在 api/
会变成 core → api 的层级倒置。这里全是令牌生命周期，本来就属于鉴权基础设施，
"mobile" 只是说明它服务于 APK 那条登录链路。
"""
...
from backend.models.user_data import MobileSession, User

# 已收编到配置中心。切换前逐项对拍过：1800 / 90 / "xinya-mobile-session"，
# 与原来 os.environ.get(...) 的取值完全一致，所以在飞的 token 一个都没失效。
# ★ TOKEN_SALT 改一个字符，全体 access_token 立刻失效 —— 要改就得配合一次强制重登。
ACCESS_TOKEN_SECONDS = settings.mobile_access_token_seconds
REFRESH_TOKEN_DAYS = settings.mobile_refresh_token_days
TOKEN_SALT = settings.mobile_access_token_salt
