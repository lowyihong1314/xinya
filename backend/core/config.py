...
PROJECT_ROOT = Path(__file__).resolve().parents[2]  # backend/core/config.py → 仓库根
...
class Settings(BaseSettings):
    ...
    # read_bill_api 网关的 Bearer token。当前**只被注释掉的代码**引用
    # (backend/app/account/services.py 的 _read_bill_remote)，搬过来是为了
    # .flaskenv 退役后这个值不会丢——重新启用那段代码时不用再去翻历史。
    read_bill_admin_token: str = ""
    ...
    # 规则 token：zone 级「Email Routing Rules: Edit」。留空就回落到上面的地址 token
    # ——现网一直是空的（.flaskenv 里根本没有 Email_Routing_Rules_API_KEY_Cloudflare），
    # 所以回落是**当前的实际行为**，不是新加的容错。地址 token 对 zone 级规则返 403，
    # 表现为「目的地址能加、转发规则建不了」，configure_forwarding 会带 warning 回去。
    cloudflare_email_routing_rules_token: str = Field(
        "",
        validation_alias=AliasChoices(
            "CLOUDFLARE_EMAIL_ROUTING_RULES_TOKEN", "Email_Routing_Rules_API_KEY_Cloudflare"
        ),
    )
...
# ─────────────────────────── 迁移期兼容垫片 ───────────────────────────
def env_value(name):
    """按**环境变量名**取值，优先走 settings —— 原 app/email/service.py 的同名函数。

    ⚠️ 这是迁移期的临时垫片，给还没搬到 backend/api/ 的模块用
    （quiz_game/ai.py、fahui/YLP/paiwei_ocr.py、event/services.py、
      account/read_bill_ark.py、account/services.py、form/ai_grouping.py）。
    **搬一个模块就把它那处改成直接读 settings.xxx**，全改完后删掉本函数。

    原实现是「先 os.environ，再手工解析 .flaskenv」。现在配置统一由
    pydantic-settings 读 system_config.env，所以这里改成先查 settings 的同名字段
    （大小写无关，VITE_ 前缀会被剥掉——前端变量名混进了后端配置），
    查不到再回落 os.environ。行为差别：.flaskenv 不再是配置来源。
    """
    field = name.lower()
    if field.startswith("vite_"):
        field = field[len("vite_"):]
